...
test.describe.only('Form Layouts page', () => {
  ...
  test('input fields', async ({ page }) => {
    ...
    await usingTheGridEmailInput.pressSequentially('testy@test.com', {
      delay: 500,
    });

    //generic assertion
    const inputValue = await usingTheGridEmailInput.inputValue();
    expect(inputValue).toEqual('testy@test.com1');

    //locator assertion
    await expect(usingTheGridEmailInput).toHaveValue('testy@test.com');
  });
  ...
  test('radio buttons', async ({ page }) => {
    ...
    // await usingTheGridForm.getByRole('radio', { name: 'Option 2' }).check({ force: true });
    // expect(await radioStatus).toBeFalsy;
...
test('dialog boxes', async ({ page }) => {
  ...
  const table = await page.getByRole('table').locator('tr', { hasText: 'mdo@gmail.com' }).locator('.nb-trash').click();
